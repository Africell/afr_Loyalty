...
func (Uc *UserControl) Loyalty_Point_Expiry_Rules_Add(Login string, request Loyalty_Point_Expiry_Rules_AddRequest) (Id int64, err error) {
	if err = validateExpiryRules(request); err != nil {
		return Id, err
	}
	//check key if filled and if already used
	if request.Key == "" {
	...
	NewEntry.Opted_In_Rule_Type = request.Opted_In_Rule_Type
	NewEntry.Opted_Out_Rule_Type = request.Opted_Out_Rule_Type
	NewEntry.Opted_Out_Validity_Unit = request.Opted_Out_Validity_Unit
	NewEntry.Opted_Out_Validity_Duration = request.Opted_Out_Validity_Duration
	//add to cache and DB
	{
		putCtx, putCancel := context.WithTimeout(context.Background(), 10*time.Second)
...
func (Uc *UserControl) Loyalty_Point_Expiry_Rules_Edit(Login string, request Loyalty_Point_Expiry_Rules_EditRequest) (Id int64, err error) {
	if err = validateExpiryRules(Loyalty_Point_Expiry_Rules_AddRequest{
		Opted_In_Rule_Type:          request.Opted_In_Rule_Type,
		Validity_Unit:               request.Validity_Unit,
		Validity_Duration:           request.Validity_Duration,
		Grace_Validity_Unit:         request.Grace_Validity_Unit,
		Grace_Validity_Duration:     request.Grace_Validity_Duration,
		Opted_Out_Rule_Type:         request.Opted_Out_Rule_Type,
		Opted_Out_Validity_Unit:     request.Opted_Out_Validity_Unit,
		Opted_Out_Validity_Duration: request.Opted_Out_Validity_Duration,
	}); err != nil {
		return Id, err
	}
	//check and validate
	...
	entry.Opted_In_Rule_Type = request.Opted_In_Rule_Type
	entry.Opted_Out_Rule_Type = request.Opted_Out_Rule_Type
	entry.Opted_Out_Validity_Unit = request.Opted_Out_Validity_Unit
	entry.Opted_Out_Validity_Duration = request.Opted_Out_Validity_Duration
	if request.NewKey != "" {
		if request.NewKey != request.Key {
			//delete old
			{
...
func (Uc *UserControl) Customer_UAT_Add(Login string, request Customer_UAT_AddRequest) (Id int64, err error) {
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_UAT](chkCtx, Customer_UAT{Key: request.Key}.RedisKey(), Mdb_Customer_UAT, bson.M{"Key": request.Key})
		chkCancel()
		if chkErr == nil {
			err = errors.New("key already exist")
			return Id, err
...
func (Uc *UserControl) Customer_UAT_Edit(Login string, request Customer_UAT_EditRequest) (Id int64, err error) {
	...
	entry, entryErr := getJSONWithMongoFallback[Customer_UAT](context.Background(), Customer_UAT{Key: request.Key}.RedisKey(), Mdb_Customer_UAT, bson.M{"Key": request.Key})
	if redisx.IsNil(entryErr) {
		err = errors.New("key is not created")
		return Id, err
	}
...
func (Uc *UserControl) Customer_UAT_Get(Key string) (entries []Customer_UAT, err error) {
	if Key == "" {
		...
		entry, entryErr := getJSONWithMongoFallback[Customer_UAT](context.Background(), Customer_UAT{Key: Key}.RedisKey(), Mdb_Customer_UAT, bson.M{"Key": Key})
		if redisx.IsNil(entryErr) {
			err = errors.New("key does not exist")
			return entries, err
		}
...
func (Uc *UserControl) Customer_DND_Add(Login string, request Customer_DND_AddRequest) (Id int64, err error) {
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_DND](chkCtx, Customer_DND{Key: request.Key}.RedisKey(), Mdb_Customer_DND, bson.M{"Key": request.Key})
		chkCancel()
		if chkErr == nil {
			err = errors.New("key already exist")
			return Id, err
...
func (Uc *UserControl) Customer_DND_Edit(Login string, request Customer_DND_EditRequest) (Id int64, err error) {
	...
	entry, entryErr := getJSONWithMongoFallback[Customer_DND](context.Background(), Customer_DND{Key: request.Key}.RedisKey(), Mdb_Customer_DND, bson.M{"Key": request.Key})
	if redisx.IsNil(entryErr) {
		err = errors.New("key is not created")
		return Id, err
	}
...
func (Uc *UserControl) Customer_DND_Get(Key string) (entries []Customer_DND, err error) {
	if Key == "" {
		...
		entry, entryErr := getJSONWithMongoFallback[Customer_DND](context.Background(), Customer_DND{Key: Key}.RedisKey(), Mdb_Customer_DND, bson.M{"Key": Key})
		if redisx.IsNil(entryErr) {
			err = errors.New("key does not exist")
			return entries, err
		}
...
func (Uc *UserControl) Customer_Exclusion_Add(Login string, request Customer_Exclusion_AddRequest) (Id int64, err error) {
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_Exclusion](chkCtx, Customer_Exclusion{Key: request.Key}.RedisKey(), Mdb_Customer_Exclusion, bson.M{"Key": request.Key})
		chkCancel()
		if chkErr == nil {
			err = errors.New("key already exist")
			return Id, err
...
func (Uc *UserControl) Customer_Exclusion_Edit(Login string, request Customer_Exclusion_EditRequest) (Id int64, err error) {
	...
	entry, entryErr := getJSONWithMongoFallback[Customer_Exclusion](context.Background(), Customer_Exclusion{Key: request.Key}.RedisKey(), Mdb_Customer_Exclusion, bson.M{"Key": request.Key})
	if redisx.IsNil(entryErr) {
		err = errors.New("key is not created")
		return Id, err
	}
...
func (Uc *UserControl) Customer_Exclusion_Get(Key string) (entries []Customer_Exclusion, err error) {
	if Key == "" {
		...
		entry, entryErr := getJSONWithMongoFallback[Customer_Exclusion](context.Background(), Customer_Exclusion{Key: Key}.RedisKey(), Mdb_Customer_Exclusion, bson.M{"Key": Key})
		if redisx.IsNil(entryErr) {
			err = errors.New("key does not exist")
			return entries, err
		}
...
func (Uc *UserControl) Customer_COS_Exclusion_Add(Login string, request Customer_COS_Exclusion_AddRequest) (Id int64, err error) {
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_COS_Exclusion](chkCtx, Customer_COS_Exclusion{Key: request.Key}.RedisKey(), Mdb_Customer_COS_Exclusion, bson.M{"Key": request.Key})
		chkCancel()
		if chkErr == nil {
			err = errors.New("key already exist")
			return Id, err
...
func (Uc *UserControl) Customer_COS_Exclusion_Edit(Login string, request Customer_COS_Exclusion_EditRequest) (Id int64, err error) {
	...
	entry, entryErr := getJSONWithMongoFallback[Customer_COS_Exclusion](context.Background(), Customer_COS_Exclusion{Key: request.Key}.RedisKey(), Mdb_Customer_COS_Exclusion, bson.M{"Key": request.Key})
	if redisx.IsNil(entryErr) {
		err = errors.New("key is not created")
		return Id, err
	}
...
func (Uc *UserControl) Customer_COS_Exclusion_Get(Key string) (entries []Customer_COS_Exclusion, err error) {
	if Key == "" {
		...
		entry, entryErr := getJSONWithMongoFallback[Customer_COS_Exclusion](context.Background(), Customer_COS_Exclusion{Key: Key}.RedisKey(), Mdb_Customer_COS_Exclusion, bson.M{"Key": Key})
		if redisx.IsNil(entryErr) {
			err = errors.New("key does not exist")
			return entries, err
		}
...
func (Uc *UserControl) Customer_Loyalty_Account_Add(Login string, request Customer_Loyalty_Account_AddRequest) (Id int64, err error) {
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_Loyalty_Account](chkCtx, Customer_Loyalty_Account{Key: request.Key}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": request.Key})
		chkCancel()
		if chkErr == nil {
			err = errors.New("key already exist")
			return Id, err
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_COS_Exclusion](chkCtx, Customer_COS_Exclusion{Key: request.COS}.RedisKey(), Mdb_Customer_COS_Exclusion, bson.M{"Key": request.COS})
		chkCancel()
		if chkErr == nil {
...
func (Uc *UserControl) Customer_Loyalty_Account_Edit(Login string, request Customer_Loyalty_Account_EditRequest) (Id int64, err error) {
	...
	entry, entryErr := getJSONWithMongoFallback[Customer_Loyalty_Account](context.Background(), Customer_Loyalty_Account{Key: request.Key}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": request.Key})
	if redisx.IsNil(entryErr) {
		err = errors.New("key is not created")
		return Id, err
	}
	...
	if Login == "DWH_Import" {
		...
		for _, pointKey := range entry.Points_Detail_Keys {
			pointsDetail, batchErr := Uc.Customer_Loyalty_Account_Points_Details_Get(pointKey)
			if batchErr != nil {
				return Id, batchErr
			}
			if len(pointsDetail) > 0 {
				batches = append(batches, pointsDetail[0])
			}
		}
		comingExpiry, resolvedInitial, resolvedPoints := resolveAccountExpiryDates(entry, expiry_Rule, batches)
...
// of truth) and re-populates the cache best-effort. 
...
// quarterEndDate returns the end date of the quarter for the given time.
func quarterEndDate(t time.Time) time.Time {
	month := int(t.Month())
	...
	// day=0 of month+1 is the last day of quarterEndMonth
	return time.Date(t.Year(), quarterEndMonth+1, 0, 23, 59, 59, 0, time.UTC)
}

func computeBatchExpiryDate(yearMonth string, rule Loyalty_Point_Expiry_Rules) (time.Time, error) {
	if len(yearMonth) != 6 {
		return time.Time{}, fmt.Errorf("invalid yearMonth %q: must be YYYYMM", yearMonth)
	}
	year, err := strconv.Atoi(yearMonth[:4])
	if err != nil {
		return time.Time{}, fmt.Errorf("invalid yearMonth year part %q: %w", yearMonth, err)
	}
	month, err := strconv.Atoi(yearMonth[4:])
	if err != nil {
		return time.Time{}, fmt.Errorf("invalid yearMonth month part %q: %w", yearMonth, err)
	}
	earnedDate := time.Date(year, time.Month(month), 1, 0, 0, 0, 0, time.UTC)
	switch rule.Opted_In_Rule_Type {
	case "Monthly":
		// last day of (earnedMonth + Validity_Duration): day=0 of the following month
		target := earnedDate.AddDate(0, rule.Validity_Duration, 0)
		return time.Date(target.Year(), target.Month()+1, 0, 23, 59, 59, 0, time.UTC), nil
	case "Quarterly":
		qEnd := quarterEndDate(earnedDate)
		// same quarter end, 1 year later
		return time.Date(qEnd.Year()+1, qEnd.Month(), qEnd.Day(), 23, 59, 59, 0, time.UTC), nil
	default:
		return time.Time{}, fmt.Errorf("computeBatchExpiryDate: unsupported rule type %q", rule.Opted_In_Rule_Type)
	}
}

// validateExpiryRules validates the fields of a Loyalty_Point_Expiry_Rules_AddRequest
func validateExpiryRules(req Loyalty_Point_Expiry_Rules_AddRequest) error {
	if req.Opted_In_Rule_Type == "" {
		return nil
	}
	...
	case "Monthly":
		...
		if req.Validity_Duration <= 0 {
			return errors.New("Monthly rule requires Validity_Duration > 0")
		}
	case "Quarterly":
	...
	case "Fixed":
		switch req.Opted_Out_Validity_Unit {
		case "Year", "Month", "Day":
		default:
			return fmt.Errorf("Fixed rule requires Opted_Out_Validity_Unit of Year, Month, or Day; got %q", req.Opted_Out_Validity_Unit)
		}
		if req.Opted_Out_Validity_Duration <= 0 {
			return errors.New("Fixed rule requires Opted_Out_Validity_Duration > 0")
		}
	case "FollowOptedIn", "":
		// ok
	default:
		return fmt.Errorf("invalid Opted_Out_Rule_Type %q: must be Fixed or FollowOptedIn", req.Opted_Out_Rule_Type)
	}
	return nil
}
...
func resolveAccountExpiryDates(account Customer_Loyalty_Account, rule Loyalty_Point_Expiry_Rules, batches []Customer_Loyalty_Account_Points_Detail) (comingExpiry time.Time, initialDate time.Time, pointsToExpire float64) {
	...
	case "Monthly", "Quarterly":
		...
		for _, b := range batches {
			batchExpiry, err := computeBatchExpiryDate(b.Year_Month, rule)
			if err != nil {
				continue
			}
			if nearestExpiry.IsZero() || batchExpiry.Before(nearestExpiry) {
				nearestExpiry = batchExpiry
				nearestPoints = b.Available_Points
			} else if batchExpiry.Equal(nearestExpiry) {
				nearestPoints += b.Available_Points // For quarterly rule, this will group the quarter's points into one batch
			}
		}
...
func (Uc *UserControl) Customer_Loyalty_Account_GetRedemption_Rules(MSISDN string) (Redemption_Rules Loyalty_Point_Redemption_Rule, err error) {
	...
	loyalty_account, loyaltyAccErr := getJSONWithMongoFallback[Customer_Loyalty_Account](context.Background(), Customer_Loyalty_Account{Key: MSISDN}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": MSISDN})
...
func (Uc *UserControl) Customer_Loyalty_RedeemRequest(request_header *Request_Header, request Loyalty_Redemption_Request, response *Loyalty_Redemption_log) {
	...
	//get loyalty account detail
	loyalty_Account, loyaltyAccErr := getJSONWithMongoFallback[Customer_Loyalty_Account](context.Background(), Customer_Loyalty_Account{Key: request.MSISDN}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": request.MSISDN})
	if redisx.IsNil(loyaltyAccErr) {
		response.Status = "failed"
		response.StatusCode = http.StatusBadRequest
...
func (Uc *UserControl) Loyalty_AccountCreditPoints(request_header *Request_Header, request Loyalty_AccountCreditPoints_Request, response *Loyalty_AccountCreditPoints_log, refund ...bool) {
	...
	if request.MSISDN == "" {
		...
		response.StatusDate = time.Now()
		response.E2E_Elapsedtime = (time.Since(response.ReceiveDate).Nanoseconds()) / 1000000
		//Uc.Write_Loyalty_AccountCreditPoints_log(*response); enqueue_Loyalty_AccountCreditPoints_log(*response)
		return
	}
	//fill the request info
	response.MSISDN = request.MSISDN
	...
	if redisx.IsNil(loyaltyAccErr) {
		...
		enqueue_Loyalty_AccountCreditPoints_log(*response)
		return
	}
	if loyaltyAccErr != nil {
		response.Status = "failed"
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_Exclusion](chkCtx, Customer_Exclusion{Key: loyalty_account.Key}.RedisKey(), Mdb_Customer_Exclusion, bson.M{"Key": loyalty_account.Key})
		chkCancel()
		if chkErr == nil {
			response.Status = "failed"
			response.StatusCode = http.StatusBadRequest
	...
	{
		chkCtx, chkCancel := context.WithTimeout(context.Background(), 10*time.Second)
		_, chkErr := getJSONWithMongoFallback[Customer_COS_Exclusion](chkCtx, Customer_COS_Exclusion{Key: loyalty_account.COS}.RedisKey(), Mdb_Customer_COS_Exclusion, bson.M{"Key": loyalty_account.COS})
		chkCancel()
		if chkErr == nil {
			response.Status = "failed"
			response.StatusCode = http.StatusBadRequest
	...
	if points > 0 {
		...
		PointsDetail, pdErr := getJSONWithMongoFallback[Customer_Loyalty_Account_Points_Detail](context.Background(), Customer_Loyalty_Account_Points_Detail{Key: request.MSISDN + "|" + YYYY + MM}.RedisKey(), Mdb_Customer_Loyalty_Account_Points_Detail, bson.M{"Key": request.MSISDN + "|" + YYYY + MM})
...
func (Uc *UserControl) Loyalty_AccountDebitPoints(request_header *Request_Header, request Loyalty_AccountDebitPoints_Request, response *Loyalty_AccountDebitPoints_log, notRedemption ...bool) {
	...
	if request.MSISDN == "" {
		...
		enqueue_Loyalty_AccountDebitPoints_log(*response)
		return
	}
	//fill the request info
	response.MSISDN = request.MSISDN
...
func (Uc *UserControl) Customer_Loyalty_OptRequest(request_header *Request_Header, request Loyalty_Opt_Request, response *Loyalty_Status_log) {
	...
	if request.MSISDN == "" {
		...
		enqueue_Loyalty_Status_log(*response)
		return
	}
	response.StatusDate = time.Now()
	//fill the request header info
	...
	if loyalty_account.First_Opt_In_Status_Date.IsZero() && request.Opt_Status == "OptedIn" {
		...
		loyalty_account, loyaltyAccErr2 := getJSONWithMongoFallback[Customer_Loyalty_Account](context.Background(), Customer_Loyalty_Account{Key: request.MSISDN}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": request.MSISDN})
...
func (Uc *UserControl) Customer_Loyalty_Account_GetAwardedPoints(startDate, endDate time.Time) (response map[string]LoyaltySummary, err error) {
	...
	for d := startDate; !d.After(endDate); d = d.AddDate(0, 0, 1) {
		...
		for cursor.Next(context.Background()) {
			...
			cusAccount, caErr := getJSONWithMongoFallback[Customer_Loyalty_Account](context.Background(), Customer_Loyalty_Account{Key: doc.MSISDN}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": doc.MSISDN})
...
func (Uc *UserControl) EvaluateAndUpdate_CustomerLoyaltyLevel(Login string, Account_Key string) (New_Loyalty_Level_Key string, err error) {
	loyalty_account, laErr := getJSONWithMongoFallback[Customer_Loyalty_Account](context.Background(), Customer_Loyalty_Account{Key: Account_Key}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": Account_Key})
...
func (Uc *UserControl) PointsExpiry_ProcessExec(account Customer_Loyalty_Account) {
	...
	expiry_Rule, expiryRuleErr := redisx.GetJSON[Loyalty_Point_Expiry_Rules](context.Background(), RedisClient, Loyalty_Point_Expiry_Rules{Key: plan.Expiry_Rules_Key}.RedisKey())
	if redisx.IsNil(expiryRuleErr) {
		monthly_expiry_log.ExpiryStatus = "failed"
		monthly_expiry_log.ExpiryStatusDescription = "expiry rule not found"
		Uc.Write_Loyalty_Monthly_Expiry_log(monthly_expiry_log)
		<-chan_PointsExpiry_Controler
		return
	}
	if expiryRuleErr != nil {
		monthly_expiry_log.ExpiryStatus = "failed"
		monthly_expiry_log.ExpiryStatusDescription = "type assertion issue with expiry rule"
		Uc.Write_Loyalty_Monthly_Expiry_log(monthly_expiry_log)
		<-chan_PointsExpiry_Controler
		return
	}
	...
	for _, pointKey := range pointKeysSnapshot {
		...
		default:
			...
			if monthErr != nil {
				monthly_expiry_log.ExpiryStatus = "failed"
				monthly_expiry_log.ExpiryStatusDescription = "invalid year_month format"
				Uc.Write_Loyalty_Monthly_Expiry_log(monthly_expiry_log)
				<-chan_PointsExpiry_Controler
				return
			}
		...
		entry, entryErr := getJSONWithMongoFallback[Customer_Loyalty_Account](context.Background(), Customer_Loyalty_Account{Key: account.Key}.RedisKey(), Mdb_Customer_Loyalty_Account, bson.M{"Key": account.Key})
		if redisx.IsNil(entryErr) {
			err = errors.New("key does not exist")
